...
func main() {
	...
	simResp, err := suiClient.SuiDryRunTransactionBlock(ctx, models.SuiDryRunTransactionBlockRequest{TxBytes: req.TxBytes})
	if err != nil {
		fmt.Printf("Error simulating transaction: %v\n", err)
		return
	}

	gasBudget, err := utils.GetGasUsedFromSimulationResponse(&simResp)
	if err != nil {
		fmt.Printf("Error getting gas used from simulation response: %v\n", err)
		return
	}
